...
describe("Custom Error Types", () => {
  describe("ConnectionError", () => {
    it("should create connection error with proper properties", () => {
      const error = new ConnectionError("Connection failed", ErrorCode.CONNECTION_FAILED);
      
      expect(error.name).toBe("ConnectionError");
      expect(error.message).toBe("Connection failed");
      expect(error.code).toBe(ErrorCode.CONNECTION_FAILED);
      expect(error.isRetryable).toBe(true);
      expect(error.getUserMessage()).toBe("Unable to connect to the server. Please try again later.");
    });

    it("should provide specific user messages for different connection codes", () => {
      const timeoutError = new ConnectionError("Timeout", ErrorCode.CONNECTION_TIMEOUT);
      expect(timeoutError.getUserMessage()).toBe("Connection timed out. Please check your network connection and try again.");

      const refusedError = new ConnectionError("Refused", ErrorCode.CONNECTION_REFUSED);
      expect(refusedError.getUserMessage()).toBe("Connection was refused. The server may be unavailable.");
    });
  });

  describe("AuthenticationError", () => {
    it("should create authentication error with proper properties", () => {
      const error = new AuthenticationError("Invalid credentials", ErrorCode.AUTH_INVALID_CREDENTIALS);
      
      expect(error.name).toBe("AuthenticationError");
      expect(error.message).toBe("Invalid credentials");
      expect(error.code).toBe(ErrorCode.AUTH_INVALID_CREDENTIALS);
      expect(error.isRetryable).toBe(false);
      expect(error.getUserMessage()).toBe("Invalid email or password. Please check your credentials.");
    });
  });
  ...
  describe("ValidationError", () => {
    it("should create validation error with field information", () => {
      ...
      expect(error.isRetryable).toBe(false);
      expect(error.getUserMessage()).toBe("Invalid value for field 'email': Invalid email format");
    });
  });

  describe("EmailError", () => {
    it("should create email error with email-specific properties", () => {
      const error = new EmailError("Email not found", ErrorCode.EMAIL_NOT_FOUND, "123", "INBOX");
      
      expect(error.name).toBe("EmailError");
      expect(error.emailId).toBe("123");
      expect(error.folder).toBe("INBOX");
      expect(error.getUserMessage()).toBe("The requested email could not be found.");
    });
  });

  describe("CalendarError", () => {
    it("should create calendar error with calendar-specific properties", () => {
      const error = new CalendarError("Event not found", ErrorCode.EVENT_NOT_FOUND, "cal1", "event1");
      
      expect(error.name).toBe("CalendarError");
      expect(error.calendarId).toBe("cal1");
      expect(error.eventId).toBe("event1");
      expect(error.getUserMessage()).toBe("The requested calendar event could not be found.");
    });
  });
  ...
  describe("RateLimitError", () => {
    it("should create rate limit error with retry information", () => {
      ...
      expect(error.isRetryable).toBe(true);
      expect(error.getUserMessage()).toBe("Rate limit exceeded. Please try again in 60 seconds.");
    });
  });
  ...
  describe("ErrorUtils", () => {
    ...
    it("should get user-friendly messages", () => {
      ...
      expect(ErrorUtils.getUserMessage(connectionError)).toBe("Unable to connect to the server. Please try again later.");
      expect(ErrorUtils.getUserMessage(regularError)).toBe("An unexpected error occurred. Please try again.");
    });
    ...
    it("should extract error codes correctly", () => {
      ...
      expect(ErrorUtils.getErrorCode(regularError)).toBe(ErrorCode.INTERNAL_ERROR);
    });
  });

  describe("Error Serialization", () => {
    it("should serialize errors to JSON correctly", () => {
      const error = new ConnectionError("Test error", ErrorCode.CONNECTION_FAILED, {
        operation: "test",
        service: "TestService"
      });
